...
import { BookOpen, Pin, Plus, Trash2, Tag, Search, Edit3 } from "lucide-react";
import { NoteItem } from "../types";

export const NotesView: React.FC = () => {
  const { notes, createNote, deleteItem } = useApp();
  ...
  return (
    <div className="space-y-6 pb-12">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-4">
        <div>
          <h2 className="text-xl font-bold text-white tracking-tight">Executive Notes & Minutes</h2>
      ...
      {isAdding && (
        <form
          ...
          className="p-5 rounded-2xl bg-slate-900 border border-blue-500/30 space-y-3 text-xs max-w-2xl shadow-xl"
        >
          <h4 className="font-bold text-white text-sm">Create New Note</h4>
          <input
            type="text"
            required
            value={newTitle}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {filteredNotes.map((note) => (
          <div
            key={note.id}
            className={`p-5 rounded-2xl bg-slate-900/90 border transition-all text-xs space-y-3 flex flex-col justify-between ${
              note.pinned
            ...
            <div className="pt-3 border-t border-slate-800/60 flex items-center justify-between text-[11px] text-slate-400">
              <div className="flex items-center gap-1.5">
                {note.tags.map((t) => (
                  <span
                    key={t}
                    className="px-2 py-0.5 rounded-md bg-slate-800 text-blue-400 font-mono text-[10px]"
                  >
                    #{t}
                  </span>
                ))}
